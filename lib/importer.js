...
function importFiles(project, callback) {
	var opt = {cwd: project.root};
	async.waterfall([
		function(callback) {
			glob(project.files || '**/*.*', opt, callback);
		},
		function(files, callback) {
			if (!project.ignoreFiles) {
				return callback(null, files);
			}

			if (typeof project.ignoreFiles === 'function') {
				return callback(null, files.filter(project.ignoreFiles));
			}

			// находим файлы, который нужно игнорировать, и исключаем их
			// из выборки
			var ignored = Array.isArray(project.ignoreFiles) ? project.ignoreFiles : [project.ignoreFiles];
			var result = [];

			async.each(ignored, function(pattern, callback) {
				...
				callback(err, files.filter(function(f) {
					return result.indexOf(f) === -1;
				}));
			});
		},
		function(files, callback) {
